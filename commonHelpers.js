import { a as L, S as w, i as o } from "./assets/vendor-550cebad.js";
(function () {
...
async function y(e, a) {
  const t =
      "https://pixabay.com/api/" + "?key=43036012-df3a1e6422cb462a655402953",
    r = {
      q: e,
      image_type: "photo",
  ...
  return (await L.get(t, { params: r })).data;
}
let p = new w(".gallery a", { captionDelay: 250, captionsData: "alt" });
const S = document.querySelector(".gallery");
function b(e) {
...
function h(e) {
  ...
  S.insertAdjacentHTML("beforeend", a),
    p.on("show.simplelightbox", function () {}),
    p.refresh();
}
const q = document.querySelector(".gallery"),
  x = document.querySelector(".search-form"),
  g = document.querySelector("input"),
  u = document.querySelector("#load-more");
...
async function E(e) {
  if (
    (e.preventDefault(), (q.innerHTML = ""), (i = 1), (l = g.value.trim()), !l)
  ) {
    c(),
      o.show({
        title: "Warning",
  ...
  try {
    ...
    (f = Math.ceil(a.totalHits / R)), h(a.hits);
  } catch {
    (s.style.display = "none"),
      c(),
      o.error({
  ...
  v(), (s.style.display = "none"), (g.value = "");
}
async function M() {
  (s.style.display = "block"), (i += 1);
  try {
    const e = await y(l, i);
    h(e.hits);
  } catch {
